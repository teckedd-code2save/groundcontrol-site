...
const features = [
  { title: "Deployments", copy: "Build and deploy from connected repositories to Docker Compose, static sites, k3s, Cloud Run and Terraform-backed targets.", meta: "Build · deploy · verify · redeploy" },
  ...
  { title: "Terminal", copy: "Use a real browser terminal against the VPS, including host-level commands when GroundControl itself is running in Docker.", meta: "Shell · host operations · AI commands" },
  { title: "Intelligence", copy: "Investigate broken customer journeys from live evidence, connect changes to failures and prepare the next safe action.", meta: "Evidence · investigation · recovery" },
    ...
    <main id="top">
      ...
      <section className="hero section-shell">
        <div className="hero-copy-block">
          ...
          <p className="hero-copy">GroundControl gives people and AI agents controlled access to deployment, runtime, terminal, domains and infrastructure operations without handing the conversation your server credentials.</p>
          ...
          <div className="hero-notes"><span>Open source</span><span>Self-hosted</span><span>Docker native</span><span>Agent ready</span></div>
        </div>

        <aside className="hero-session" aria-label="GroundControl real workflow example">
          <p className="eyebrow">REAL WORKFLOW · CHATGPT × GROUND CONTROL</p>
          <h2>Authenticate once, then keep the work inside the conversation.</h2>
          <div className="session-steps">
            <p><span>01</span> ChatGPT identifies GroundControl&apos;s login flow and brings up authentication.</p>
            <p><span>02</span> You sign in to your GroundControl instance. Server credentials are not pasted into chat.</p>
            <p><span>03</span> ChatGPT uses the tools GroundControl exposes to build, deploy, inspect and operate services.</p>
            <p><span>04</span> GroundControl&apos;s onboard terminal and live host state stay available for diagnosis and verification.</p>
          </div>
          <a href={PRODUCT_URL}>Open the live product <Arrow /></a>
        </aside>
      </section>

      <section className="proof-band"><div className="section-shell proof-grid"><p className="eyebrow">WHY GROUND CONTROL</p><div><h2>No local build box required.</h2><p>Work can continue through deployments, runtime inspection, domains and the terminal without spinning up resources on your laptop, manually SSHing for every step, or jumping into GitHub just to trigger a deployment.</p></div></div></section>
      ...
      <section className="workflow-section"><div className="section-shell workflow-grid">
        <div className="section-heading workflow-copy"><p className="eyebrow">FROM REQUEST TO RUNNING SERVICE</p><h2>Less tab-switching. More finished work.</h2><p>Use the GroundControl UI directly, or let an authenticated agent work through the tools GroundControl exposes. Repositories, build context, deployment targets, live host state and the terminal meet in one operating surface.</p></div>
      ...
      <section className="ownership-section"><div className="section-shell ownership-grid"><p className="eyebrow">CAPABILITIES, NOT CREDENTIALS</p><h2>Your infrastructure credentials stay with GroundControl. The client gets the operations you choose to expose.</h2><p>GroundControl is single-tenant and self-hosted. It can run on the same VPS it manages, talk to additional hosts, and perform host-level work through its controlled execution layer.</p></div></section>
